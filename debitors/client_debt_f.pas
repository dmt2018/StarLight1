...
begin
  ...
  begin
    if (f_begin.EditValue > f_end.EditValue) then
       MessageBox(Handle, 'Необходимо чтобы начальная дата была меньше конечной!', 'Внимание', MB_ICONWARNING)
    else
    begin
      CDS_CLIENT_DEBT.ParamByName('DATE_BEGIN_').AsDate := f_begin.Date;
      CDS_CLIENT_DEBT.ParamByName('DATE_END_').AsDate := f_end.Date;
...
// 2012-01-19 ЕВ просила, чтобы только для рублевых
//    if (gr_debetors_v.DataController.DataSet.FieldByName('BEZNAL').AsInteger = 1) or (gr_debetors_v.DataController.DataSet.FieldByName('RUBL').AsInteger = 1) then
    ...
    client_debt_edit.Panel1.Caption := 'Добавление записи в дебиторскую карточку';
...
begin
  ...
  try
    ...
    client_debt_edit.Panel1.Caption := 'Редактирование записи в дебиторской карточке';
    client_debt_edit.showmodal;
...
//
//  По Ctrl+Tab меняем фокус у таблиц
//
...
begin
  ...
  try
    if MessageDlg('Вы действительно хотите удалить выделенные записи?',mtConfirmation,[mbYes, mbNo],0) = mrYes then
    begin
...
begin
  ...
  begin
     ...
     try
       ...
       on E: Exception do
          MessageBox(Handle, PChar(E.Message), 'Ошибка при обновлении информации!', MB_ICONERROR);
     End;
...
begin
  ...
  DM.Q_SQL.Open;
  MessageBox(Handle, PChar('USD - ' + DM.Q_SQL.FieldByName('SHEV_USD').AsString + #10 + 'EUR - ' + DM.Q_SQL.FieldByName('SHEV_EUR').AsString ), PChar('Курсы на дату '+cds.FieldByName('D_DATE').AsString), MB_ICONINFORMATION);
  DM.Q_SQL.Close;
end;
...
{
 Подстановка возврата долга под нужный день
 Расчет даты, на которую выплаченный долг закроет сумму задолженности.
 Операция только для валютных клиентов.
 Если встречаемые курсы долговых операций меньше текущего курса, то берется текущий курс.
}
procedure Tclient_debt.mmSetDolgClick(Sender: TObject);
begin
  if CDS_CLIENT_DEBTOPERATION.AsInteger <> 3 then
  begin
     MessageBox(Handle, 'Необходимо выбрать операцию ДОЛГ', 'Внимание!', MB_ICONWARNING);
     exit;
  end;
