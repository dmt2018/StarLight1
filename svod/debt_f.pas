           ...
           begin
             debet := RoundTo(
              ...
              begin
                ...
                if (DM.Q_CASH_SIN_RUB.AsCurrency > 0) and (AnsiUpperCase(DM.Q_CASHINFO.AsString) = 'П/К') then debet := 0-DM.Q_CASH_SIN_RUB.AsCurrency;
              end;
           end
           else
        ...
        begin
           debet := RoundTo(
            ...
            begin
              ...
              //2016-06-24 Если в комметарии стоит П/К, то это ЧАСТЬ ДОЛГА НАЛ, ЧАСТЬ - ПЛАСТИК. Расчет только по IN
              if (IN_RUB_ > 0) and (AnsiUpperCase(DM.Q_CASH_SINFO.AsString) = 'П/К') then debet := 0-IN_RUB_;
            end;
        end
        else
