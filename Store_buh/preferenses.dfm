object preferensesForm: TpreferensesForm
  Left = 601
  Top = 224
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  BorderWidth = 5
  Caption = #1053#1072#1089#1090#1088#1086#1081#1082#1080
  ClientHeight = 556
  ClientWidth = 404
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'Arial'
  Font.Pitch = fpFixed
  Font.Style = []
  Icon.Data = {
    
    0000000100000001000000FF000001FF0000FFFF0000}
  OldCreateOrder = False
  Position = poOwnerFormCenter
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 16
  object bv_main: TBevel
    Left = 0
    Top = 519
    Width = 404
    Height = 2
    Align = alBottom
    ExplicitTop = 353
    ExplicitWidth = 383
  end
  object Panel2: TPanel
    Left = 0
    Top = 521
    Width = 404
    Height = 35
    Align = alBottom
    BevelOuter = bvNone
    Ctl3D = False
    ParentCtl3D = False
    TabOrder = 3
    DesignSize = (
      404
      35)
    object BitBtn_Cancel: TcxButton
      Left = 121
      Top = 3
      Width = 140
      Height = 30
      Cursor = crHandPoint
      Anchors = [akTop, akRight]
      Cancel = True
      Caption = ' '#1054#1090#1084#1077#1085#1072
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = [fsBold]
      ModalResult = 2
      ParentFont = False
      TabOrder = 0
      Colors.Default = clBtnFace
      Colors.Normal = clBtnFace
      Colors.Hot = clSkyBlue
      Colors.Pressed = clSkyBlue
      Glyph.Data = {
        
        0000000000002828A0312B2BAE891B1B6F020000000000000000}
      LookAndFeel.Kind = lfFlat
      LookAndFeel.NativeStyle = True
      Spacing = 6
    end
    object BitBtnSave: TcxButton
      Left = 262
      Top = 3
      Width = 140
      Height = 30
      Cursor = crHandPoint
      Anchors = [akTop, akRight]
      Caption = #1057#1086#1093#1088#1072#1085#1080#1090#1100
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      OnClick = BitBtnFilterOnClick
      Colors.Default = clBtnFace
      Colors.Normal = clBtnFace
      Colors.Hot = clSkyBlue
      Colors.Pressed = clSkyBlue
      Glyph.Data = {
        
        6666666666666666666666666666666666666666666666666666}
      LookAndFeel.Kind = lfFlat
      LookAndFeel.NativeStyle = True
      Spacing = 6
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 394
    Width = 404
    Height = 119
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Pitch = fpFixed
    Font.Style = [fsBold]
    ParentFont = False
    Style = tsFlatButtons
    TabOrder = 2
  end
  object GroupBox2: TGroupBox
    Left = 0
    Top = 31
    Width = 404
    Height = 90
    Align = alTop
    Caption = ' '#1048#1089#1087#1086#1083#1100#1079#1091#1077#1084#1099#1077' '#1088#1077#1082#1074#1080#1079#1080#1090#1099' '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Pitch = fpFixed
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    DesignSize = (
      404
      90)
    object Label1: TLabel
      Left = 7
      Top = 28
      Width = 74
      Height = 15
      Caption = #1050#1086#1084#1087#1072#1085#1080#1103' (*)'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 35
      Top = 60
      Width = 44
      Height = 15
      Caption = #1041#1072#1085#1082' (*)'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
    end
    object Company_All_CBEx: TDBComboBoxEh
      Left = 88
      Top = 24
      Width = 303
      Height = 23
      Cursor = crHandPoint
      Anchors = [akLeft, akTop, akRight]
      EditButtons = <>
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      Visible = True
      OnChange = DBComboBoxEh1Change
    end
    object Banc_All_CBEx: TDBComboBoxEh
      Left = 88
      Top = 56
      Width = 303
      Height = 23
      Cursor = crHandPoint
      Anchors = [akLeft, akTop, akRight]
      EditButtons = <>
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Visible = True
    end
  end
  object GroupBox6: TGroupBox
    Left = 0
    Top = 127
    Width = 404
    Height = 122
    Align = alTop
    Caption = #1044#1088#1091#1075#1080#1077' '#1085#1072#1089#1090#1088#1086#1081#1082#1080
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Pitch = fpFixed
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    object Label5: TLabel
      Left = 12
      Top = 25
      Width = 110
      Height = 15
      Caption = #1053#1044#1057' '#1087#1086' '#1091#1084#1086#1083#1095#1072#1085#1080#1102
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
    end
    object Label7: TLabel
      Left = 200
      Top = 23
      Width = 16
      Height = 19
      Caption = '%'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbOKPO: TLabel
      Left = 240
      Top = 27
      Width = 34
      Height = 15
      Caption = #1054#1050#1055#1054
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
    end
    object lbOKUD: TLabel
      Left = 240
      Top = 51
      Width = 31
      Height = 15
      Caption = #1054#1050#1059#1044
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
    end
    object lbOKDP: TLabel
      Left = 240
      Top = 75
      Width = 33
      Height = 15
      Caption = #1054#1050#1044#1055
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
    end
    object Nds_All_UD: TUpDown
      Left = 178
      Top = 23
      Width = 15
      Height = 21
      Associate = Nds_All_Edit
      Max = 99
      TabOrder = 0
    end
    object Nds_All_Edit: TMaskEdit
      Left = 131
      Top = 23
      Width = 47
      Height = 21
      AutoSelect = False
      BevelEdges = []
      BevelInner = bvNone
      BevelOuter = bvNone
      Ctl3D = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      MaxLength = 2
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 1
      Text = '0'
    end
    object chbNDS: TCheckBox
      Left = 12
      Top = 51
      Width = 131
      Height = 17
      Alignment = taLeftJustify
      Caption = #1041#1077#1079' '#1053#1044#1057
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      TabOrder = 2
    end
    object edOKPO: TcxTextEdit
      Left = 288
      Top = 23
      Style.TextColor = clWindowText
      TabOrder = 3
      Text = 'edOKPO'
      Width = 103
    end
    object edOKUD: TcxTextEdit
      Left = 288
      Top = 47
      Style.TextColor = clWindowText
      TabOrder = 4
      Text = 'edOKUD'
      Width = 103
    end
    object edOKDP: TcxTextEdit
      Left = 288
      Top = 71
      Style.TextColor = clWindowText
      TabOrder = 5
      Text = 'edOKDP'
      Width = 103
    end
    object chbNDSMinus: TCheckBox
      Left = 12
      Top = 75
      Width = 131
      Height = 17
      Alignment = taLeftJustify
      Caption = #1059#1084#1077#1085#1100#1096#1080#1090#1100' '#1085#1072' '#1053#1044#1057
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      TabOrder = 6
    end
    object chbSpecDiscont: TCheckBox
      Left = 12
      Top = 98
      Width = 270
      Height = 17
      Alignment = taLeftJustify
      Caption = #1055#1088#1080#1084#1077#1085#1103#1090#1100' '#1089#1082#1080#1076#1082#1091' '#1076#1083#1103' '#1089#1087#1077#1094'.'#1087#1088#1077#1076#1083#1086#1078#1077#1085#1080#1103
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ParentFont = False
      TabOrder = 7
    end
  end
  object Panel4: TPanel
    Left = 0
    Top = 0
    Width = 404
    Height = 25
    Align = alTop
    BevelInner = bvRaised
    BevelOuter = bvLowered
    Caption = #1054#1073#1097#1080#1077' '#1085#1072#1089#1090#1088#1086#1081#1082#1080
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Pitch = fpFixed
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 4
  end
  object Panel1: TPanel
    Left = 0
    Top = 261
    Width = 404
    Height = 25
    Align = alTop
    BevelInner = bvRaised
    BevelOuter = bvLowered
    Caption = #1055#1077#1088#1089#1086#1085#1072#1083#1100#1085#1099#1077' '#1085#1072#1089#1090#1088#1086#1081#1082#1080
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Pitch = fpFixed
    Font.Style = []
    ParentBackground = False
    ParentFont = False
    TabOrder = 5
  end
  object Panel3: TPanel
    Left = 0
    Top = 25
    Width = 404
    Height = 6
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 6
  end
  object Panel5: TPanel
    Left = 0
    Top = 249
    Width = 404
    Height = 12
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 7
  end
  object Panel6: TPanel
    Left = 0
    Top = 121
    Width = 404
    Height = 6
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 8
  end
  object DBGridEh2: TDBGridEh
    Left = 0
    Top = 292
    Width = 404
    Height = 96
    Align = alTop
    AllowedOperations = [alopUpdateEh, alopDeleteEh]
    AllowedSelections = [gstRecordBookmarks, gstAll]
    AutoFitColWidths = True
    ColumnDefValues.DropDownShowTitles = True
    ColumnDefValues.EndEllipsis = True
    ColumnDefValues.Title.TitleButton = True
    ColumnDefValues.Title.ToolTips = True
    ColumnDefValues.ToolTips = True
    Ctl3D = False
    DataGrouping.GroupLevels = <>
    DataSource = company_DS
    EditActions = [geaCopyEh, geaPasteEh, geaSelectAllEh]
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Pitch = fpFixed
    Font.Style = []
    FooterColor = clCream
    FooterFont.Charset = DEFAULT_CHARSET
    FooterFont.Color = clWindowText
    FooterFont.Height = -11
    FooterFont.Name = 'MS Sans Serif'
    FooterFont.Pitch = fpFixed
    FooterFont.Style = [fsBold]
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghAutoSortMarking, dghMultiSortMarking, dghRowHighlight, dghDialogFind, dghColumnResize, dghColumnMove]
    ParentCtl3D = False
    ParentFont = False
    RowDetailPanel.Color = clBtnFace
    SortLocal = True
    TabOrder = 9
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'Arial'
    TitleFont.Pitch = fpFixed
    TitleFont.Style = []
    VTitleMargin = 20
    Columns = <
      item
        EditButtons = <>
        FieldName = 'company_id'
        Footers = <>
        MaxWidth = 80
        MinWidth = 80
        Title.Caption = 'ID'
        Width = 80
      end
      item
        EditButtons = <>
        FieldName = 'COMPANY_NAME'
        Footers = <>
        Title.Caption = #1053#1072#1079#1074#1072#1085#1080#1077' '#1082#1086#1084#1087#1072#1085#1080#1080
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object Panel7: TPanel
    Left = 0
    Top = 286
    Width = 404
    Height = 6
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 10
  end
  object Panel8: TPanel
    Left = 0
    Top = 388
    Width = 404
    Height = 6
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 11
  end
  object DBGridEh1: TDBGridEh
    Left = 0
    Top = 394
    Width = 404
    Height = 119
    Align = alClient
    AllowedOperations = [alopInsertEh, alopUpdateEh, alopDeleteEh]
    AllowedSelections = [gstRecordBookmarks, gstAll]
    AutoFitColWidths = True
    ColumnDefValues.DropDownShowTitles = True
    ColumnDefValues.EndEllipsis = True
    ColumnDefValues.Title.TitleButton = True
    ColumnDefValues.Title.ToolTips = True
    ColumnDefValues.ToolTips = True
    Ctl3D = False
    DataGrouping.GroupLevels = <>
    DataSource = dsSettings
    EditActions = [geaCopyEh, geaPasteEh, geaDeleteEh, geaSelectAllEh]
    Flat = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Pitch = fpFixed
    Font.Style = []
    FooterColor = clCream
    FooterFont.Charset = DEFAULT_CHARSET
    FooterFont.Color = clWindowText
    FooterFont.Height = -11
    FooterFont.Name = 'MS Sans Serif'
    FooterFont.Pitch = fpFixed
    FooterFont.Style = [fsBold]
    Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
    OptionsEh = [dghFixed3D, dghHighlightFocus, dghClearSelection, dghAutoSortMarking, dghRowHighlight, dghDialogFind, dghColumnResize, dghColumnMove]
    ParentCtl3D = False
    ParentFont = False
    PopupMenu = pm_settings
    RowDetailPanel.Color = clBtnFace
    TabOrder = 12
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -13
    TitleFont.Name = 'Arial'
    TitleFont.Pitch = fpFixed
    TitleFont.Style = []
    VTitleMargin = 20
    OnKeyDown = DBGridEh1KeyDown
    Columns = <
      item
        EditButtons = <>
        FieldName = 'DATE_BEGIN'
        Footers = <>
        Title.Caption = #1044#1072#1090#1072' '#1087#1088#1080#1082#1072#1079#1072
        Width = 120
      end
      item
        EditButtons = <>
        FieldName = 'PRIKAZ_N'
        Footers = <>
        Title.Caption = #8470' '#1087#1088#1080#1082#1072#1079#1072
        Width = 100
      end
      item
        EditButtons = <>
        FieldName = 'DATE_END'
        Footers = <>
        Title.Caption = #1054#1082#1086#1085#1095#1072#1085#1080#1077
        Width = 120
      end>
    object RowDetailData: TRowDetailPanelControlEh
    end
  end
  object Panel9: TPanel
    Left = 0
    Top = 513
    Width = 404
    Height = 6
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 13
  end
  object banc: TOraQuery
    Session = DM.OraSession
    SQL.Strings = (
      'SELECT'
      '*'
      'from'
      'buh_banc'
      'where COMPANY_ID=:COMPANY_ID')
    MasterFields = 'COMPANY_ID'
    AutoCommit = False
    Left = 328
    Top = 8
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'COMPANY_ID'
      end>
    object bancBANC_ID: TFloatField
      FieldName = 'BANC_ID'
      Required = True
    end
    object bancCOMPANY_ID: TFloatField
      FieldName = 'COMPANY_ID'
      Required = True
    end
    object bancBANC_NAME: TStringField
      FieldName = 'BANC_NAME'
      Required = True
      Size = 255
    end
    object bancBANC_RS: TStringField
      FieldName = 'BANC_RS'
      Required = True
      Size = 255
    end
    object bancBANC_KS: TStringField
      FieldName = 'BANC_KS'
      Required = True
      Size = 255
    end
    object bancBANC_BIK: TStringField
      FieldName = 'BANC_BIK'
      Required = True
      Size = 255
    end
  end
  object banc_ds: TOraDataSource
    DataSet = banc
    Left = 328
    Top = 40
  end
  object sql_CBEx: TOraQuery
    Session = DM.OraSession
    SQL.Strings = (
      'SELECT '
      '*'
      'from'
      'books_departments')
    AutoCommit = False
    Left = 296
    Top = 8
  end
  object sql_CBEx_ds: TOraDataSource
    DataSet = sql_CBEx
    Left = 296
    Top = 40
  end
  object company: TOraQuery
    SQLRefresh.Strings = (
      'WHERE'
      '  COMPANY_ID = :COMPANY_ID')
    Session = DM.OraSession
    SQL.Strings = (
      'select'
      'a.company_id, a.company_name, a.kpp, a.inn, a.adress, a.phone,'
      
        'a.fax, a.otpusk_proizvel_fio, a.otpusk_razreshil_fio, a.glav_buh' +
        ','
      'a.otpusk_proizvel_doljn, a.otpusk_razreshil_doljn'
      'FROM '
      'BUH_company a')
    RefreshOptions = [roAfterInsert, roAfterUpdate, roBeforeEdit]
    Options.ReturnParams = True
    FilterOptions = [foCaseInsensitive]
    Left = 32
    Top = 262
  end
  object company_DS: TOraDataSource
    DataSet = company
    Left = 8
    Top = 264
  end
  object cdsSettings: TOraQuery
    SQLInsert.Strings = (
      'declare v_id number;'
      'begin'
      '  v_id := 0;'
      
        '  STORE_PKG.EDIT_PERSONAL_SETTINGS(v_id, USER, :ID_COMPANY, :DAT' +
        'E_BEGIN, :DATE_END, :PRIKAZ_N);'
      'end;')
    SQLDelete.Strings = (
      'begin'
      '  STORE_PKG.DELETE_PERSONAL_SETTINGS(:ID);'
      'end;')
    SQLUpdate.Strings = (
      'begin'
      
        '  STORE_PKG.EDIT_PERSONAL_SETTINGS(:ID, USER, :ID_COMPANY, :DATE' +
        '_BEGIN, :DATE_END, :PRIKAZ_N);'
      'end;')
    SQLRefresh.Strings = (
      
        'select a.id, a.user_name, a.id_company, a.date_begin, a.date_end' +
        ', a.prikaz_n'
      'from buh_user_settings a'
      'where a.id = :id')
    Session = DM.OraSession
    SQL.Strings = (
      
        'select a.id, a.user_name, a.id_company, a.date_begin, a.date_end' +
        ', a.prikaz_n'
      'from buh_user_settings a'
      'where a.user_name = user and a.id_company = :company_id'
      'order by a.date_begin')
    MasterSource = company_DS
    MasterFields = 'COMPANY_ID'
    DetailFields = 'ID_COMPANY'
    RefreshOptions = [roAfterUpdate]
    Options.ReturnParams = True
    FilterOptions = [foCaseInsensitive]
    AfterInsert = cdsSettingsAfterInsert
    Left = 96
    Top = 350
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'company_id'
      end>
    object cdsSettingsID: TIntegerField
      FieldName = 'ID'
      Required = True
    end
    object cdsSettingsUSER_NAME: TStringField
      FieldName = 'USER_NAME'
    end
    object cdsSettingsID_COMPANY: TIntegerField
      FieldName = 'ID_COMPANY'
    end
    object cdsSettingsDATE_BEGIN: TDateTimeField
      FieldName = 'DATE_BEGIN'
    end
    object cdsSettingsDATE_END: TDateTimeField
      FieldName = 'DATE_END'
    end
    object cdsSettingsPRIKAZ_N: TStringField
      FieldName = 'PRIKAZ_N'
    end
  end
  object dsSettings: TOraDataSource
    DataSet = cdsSettings
    Left = 96
    Top = 384
  end
  object pm_settings: TPopupMenu
    Left = 320
    Top = 376
    object mn_insert: TMenuItem
      Caption = #1044#1086#1073#1072#1074#1080#1090#1100' '#1079#1072#1087#1080#1089#1100
      OnClick = mn_insertClick
    end
    object mn_delete: TMenuItem
      Caption = #1059#1076#1072#1083#1080#1090#1100
      OnClick = mn_deleteClick
    end
  end
end
