...
uses StdCtrls, Classes, Variants, Ora, SysUtils, Graphics, OraError, Dialogs, Forms, IniFiles,
     CheckLst, windows, ComCtrls, DB, cxImageComboBox, DBGrids, cxBarEditItem, DBCtrlsEh,
     cxGridDBTableView, cxCustomData, cxCheckComboBox, ExtCtrls, Controls, cxCheckListBox;
...
// Получения имени пользователя и домена под которым работает текущий поток или процесс
  ...
  function FillCheckListCx(TheQuery : TOraQuery; TheCombo : TCxCheckListBox; BeginStr : String) : boolean;
...
function FillCheckListCx(TheQuery : TOraQuery; TheCombo : TCxCheckListBox; BeginStr : String) : boolean;
{********************************************************************************
 * Функция предназначена для заполнения списка значениями                       *
 * А также для запоминания ID каждого значения компонента DevExpress            *
 * TheQuery - TQuery с набором данных                                           *
 * TheCombo - список для значений                                               *
 * BeginStr - начальная строка в списке с ID=0, если NULL то нет первой строки  *
 * первый столбик TheQuery содержит ID , а второй - значение.                   *
 ********************************************************************************}
var //cbep: TCxCheckListBoxProperties;
    itm: TcxCheckListBoxItem;
Begin
  try
    TheQuery.First;

    //cbep := (TheCombo.Properties as TCxCheckListBoxProperties);
    TheCombo.Items.Clear;

    if Length(BeginStr) > 0 then
    begin
     	itm           := TheCombo.Items.Add;
   		itm.Text	    := BeginStr;
     	itm.Tag			  := 0;
    end;
    ...
    begin
     	itm           := TheCombo.Items.Add;
   		itm.Text	    := TheQuery.Fields[1].AsString;
    ...
    FillCheckListCx := true;
  except
    FillCheckListCx := false;
  End;
End;
