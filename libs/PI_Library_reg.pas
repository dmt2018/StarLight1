...
uses StdCtrls, Classes, Variants, Ora, SysUtils, Graphics, OraError, Dialogs,
     ...
     cxGridDBTableView, cxCustomData, cxCheckComboBox, Clipbrd, cxGridDBBandedTableView,
     cxGridTableView, cxGridDBDataDefinitions;

  // 2- Самара  10690, 12173
  // 3- Казань  14942
  // 4- Ярославль  10016021
  const
    client_kazan = 14942;
    client_samara = 10690;
...
// Разбиение текста на массив строк
function Split(f : string): TStrArray;
...
// Можно ли удалить цвет
function IsDelColor(in_col_id: integer) : boolean;
...
const ColorID: array[1..4] of Integer =  //Массив цвета, который нельзя удалять
( 0,1639,2768,2915
);
...
begin
  ...
  SetLength(a,1);
  l:=Length(f);
  while (f[n]=#10) and (n<=l)  do  // поиск первого не пробела
    n:=n+1;                        // номер символа
  for i := n to l do begin
    if f[i]=#10 then begin   // если текущий символ не пробел
      if f[i+1]<>#10 then begin   // и следующий тоже
        j := j+1;
...
begin
  ...
  // Показываем карты с изображением, которые мы имеем
   for i := 1 to High(ColorID) do
     if ColorID[i] = in_col_id then
        result := false;
end;
