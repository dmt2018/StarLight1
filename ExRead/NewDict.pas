...
begin
//  if Sender is TNewD then close; // raise Exception.Create('1'); //исключение возникло в модальной форме, еще раз вызовем исключение
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        imSubTypeWarning.Hint := 'По указанному подтипу '+or_group+' найдены несколько переводов.'#10#13+tmpSubTypes;
        MessageBox(Handle, PChar('По указанному подтипу '+or_group+' найдены несколько переводов.'#10#13+tmpSubTypes), 'Внимание', MB_ICONWARNING);
      end;
    end;
...
begin
  ...
  try
    ...
    // Если по заказу, то вставим полученные данные в форму
    if or_id > 0 then
    begin
    ...
    // Новая из текущей. Значит надо предзаполнить названия постащика
    ...
    begin
      ...
      begin
        ...
        // 2014-10-28 Е.В. просила убрать, т.к. после редактирования появляется не полное название, а простое название цветка
...
begin
  if lookcombo_name.EditValue > 0 then  
    ...
    Application.MessageBox('Необходимо выбрать название','Внимание',MB_ICONWARNING);
end;
