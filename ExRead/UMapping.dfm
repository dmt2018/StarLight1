object frmMapping: TfrmMapping
  Left = 0
  Top = 0
  Caption = #1053#1072#1089#1090#1088#1086#1081#1082#1072' '#1089#1086#1087#1086#1089#1090#1072#1074#1083#1077#1085#1080#1081
  ClientHeight = 411
  ClientWidth = 852
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 370
    Width = 852
    Height = 41
    Align = alBottom
    BevelInner = bvRaised
    BevelOuter = bvLowered
    Color = clSkyBlue
    TabOrder = 0
    DesignSize = (
      852
      41)
    object btnDelete: TcxButton
      Left = 0
      Top = 6
      Width = 225
      Height = 30
      Cursor = crHandPoint
      Caption = #1059#1076#1072#1083#1080#1090#1100' '#1089#1086#1087#1086#1089#1090#1072#1074#1083#1077#1085#1080#1077
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
      OnClick = btnDeleteClick
      Colors.Default = clBtnFace
      Colors.Normal = clBtnFace
      Colors.Hot = clSkyBlue
      Colors.Pressed = clSkyBlue
      Glyph.Data = {
        
        0000000000002828A0312B2BAE891B1B6F020000000000000000}
      LookAndFeel.Kind = lfFlat
      LookAndFeel.NativeStyle = True
      Spacing = 6
    end
    object BitBtn2: TcxButton
      Left = 672
      Top = 6
      Width = 174
      Height = 30
      Cursor = crHandPoint
      Anchors = [akTop, akRight]
      Cancel = True
      Caption = #1047#1072#1082#1088#1099#1090#1100
      Default = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Pitch = fpFixed
      Font.Style = []
      ModalResult = 2
      ParentFont = False
      TabOrder = 1
      Colors.Default = clBtnFace
      Colors.Normal = clBtnFace
      Colors.Hot = clSkyBlue
      Colors.Pressed = clSkyBlue
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      LookAndFeel.Kind = lfFlat
      LookAndFeel.NativeStyle = True
      Spacing = 6
    end
  end
  object grid_mapping: TcxGrid
    Left = 0
    Top = 0
    Width = 852
    Height = 370
    Hint = 'Ctrl+Tab '#1087#1077#1088#1077#1082#1083#1102#1095#1077#1085#1080#1077' '#1084#1077#1078#1076#1091' '#1090#1072#1073#1083#1080#1094#1072#1084#1080
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Pitch = fpFixed
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    LookAndFeel.Kind = lfOffice11
    object grid_mapping_v: TcxGridDBTableView
      NavigatorButtons.ConfirmDelete = False
      FilterBox.Position = fpTop
      DataController.DataSource = dsMapping
      DataController.Filter.Options = [fcoCaseInsensitive, fcoShowOperatorDescription, fcoSoftCompare]
      DataController.Options = [dcoAnsiSort, dcoCaseInsensitive, dcoAssignGroupingValues, dcoAssignMasterDetailKeys, dcoSaveExpanding]
      DataController.Summary.DefaultGroupSummaryItems = <
        item
          Kind = skCount
          Position = spFooter
        end
        item
          Kind = skCount
        end>
      DataController.Summary.FooterSummaryItems = <
        item
          Kind = skCount
        end
        item
          Kind = skSum
        end
        item
          Format = '0'
          Kind = skCount
        end
        item
          Format = '0'
          Kind = skSum
        end
        item
          Format = '0.00'
          Kind = skSum
        end
        item
          Format = '0'
          Kind = skSum
        end
        item
          Format = '0'
          Kind = skSum
        end
        item
          Format = '0'
          Kind = skSum
        end
        item
          Format = '0'
          Kind = skCount
          Column = grid_mapping_vAS_IS_HCODE
        end>
      DataController.Summary.SummaryGroups = <>
      FilterRow.InfoText = #1055#1086#1083#1077' '#1076#1083#1103' '#1091#1089#1090#1072#1085#1086#1074#1082#1080' '#1092#1080#1083#1100#1090#1088#1086#1074
      FilterRow.Visible = True
      OptionsBehavior.CellHints = True
      OptionsBehavior.FocusCellOnCycle = True
      OptionsCustomize.ColumnMoving = False
      OptionsData.CancelOnExit = False
      OptionsData.Deleting = False
      OptionsData.DeletingConfirmation = False
      OptionsData.Inserting = False
      OptionsView.CellEndEllipsis = True
      OptionsView.NoDataToDisplayInfoText = #1053#1077#1090' '#1076#1072#1085#1085#1099#1093' '#1076#1083#1103' '#1087#1088#1086#1089#1084#1086#1090#1088#1072
      OptionsView.ColumnAutoWidth = True
      OptionsView.Footer = True
      OptionsView.FooterMultiSummaries = True
      OptionsView.GroupByBox = False
      OptionsView.GroupFooters = gfVisibleWhenExpanded
      OptionsView.HeaderEndEllipsis = True
      object grid_mapping_vN_ID: TcxGridDBColumn
        Caption = 'ID'
        DataBinding.FieldName = 'N_ID'
        Visible = False
      end
      object grid_mapping_vREPLACEMENT: TcxGridDBColumn
        DataBinding.FieldName = 'REPLACEMENT'
        Visible = False
      end
      object grid_mapping_vAS_IS_HCODE: TcxGridDBColumn
        Caption = #1050#1086#1076' '#1080#1085#1074#1086#1081#1089#1072
        DataBinding.FieldName = 'AS_IS_HCODE'
        MinWidth = 120
        Styles.Content = cxStyle1
        Width = 120
      end
      object grid_mapping_vH_CODE: TcxGridDBColumn
        Caption = #1050#1086#1076
        DataBinding.FieldName = 'H_CODE'
        MinWidth = 120
        Width = 120
      end
      object grid_mapping_vH_NAME: TcxGridDBColumn
        Caption = #1043#1086#1083'. '#1085#1072#1079#1074#1072#1085#1080#1077
        DataBinding.FieldName = 'H_NAME'
        Width = 200
      end
      object grid_mapping_vCOMPILED_NAME_OTDEL: TcxGridDBColumn
        Caption = #1053#1072#1079#1074#1072#1085#1080#1077
        DataBinding.FieldName = 'COMPILED_NAME_OTDEL'
        Width = 300
      end
      object grid_mapping_vREMARKS: TcxGridDBColumn
        Caption = #1056#1077#1084#1072#1088#1082
        DataBinding.FieldName = 'REMARKS'
        Width = 120
      end
      object grid_mapping_vCOLOUR: TcxGridDBColumn
        Caption = #1062#1074#1077#1090
        DataBinding.FieldName = 'COLOUR'
        Width = 120
      end
    end
    object grid_mapping_l: TcxGridLevel
      GridView = grid_mapping_v
    end
  end
  object cdsMapping: TOraQuery
    SQL.Strings = (
      
        'select distinct a.n_id, a.as_is_hcode, a.replacement, n.h_code, ' +
        'n.h_name, n.compiled_name_otdel, n.remarks, n.colour'
      'from INVOICE_DATA_AS_IS_MAP a, nomenclature_mat_view n '
      'where a.as_is_hcode in ('
      '  select as_is_hcode from ('
      
        '    select as_is_hcode, count(distinct c.n_id) n_id from INVOICE' +
        '_DATA_AS_IS_MAP c '
      '    where c.replacement = 1 '
      '    group by c.as_is_hcode'
      '  ) where n_id > 1'
      ') '
      'and a.n_id = n.n_id'
      'order by a.as_is_hcode, a.n_id')
    ReadOnly = True
    Left = 128
    Top = 152
    object cdsMappingN_ID: TFloatField
      FieldName = 'N_ID'
    end
    object cdsMappingAS_IS_HCODE: TStringField
      FieldName = 'AS_IS_HCODE'
      Size = 50
    end
    object cdsMappingREPLACEMENT: TIntegerField
      FieldName = 'REPLACEMENT'
    end
    object cdsMappingH_CODE: TStringField
      FieldName = 'H_CODE'
      Size = 80
    end
    object cdsMappingH_NAME: TStringField
      FieldName = 'H_NAME'
      Size = 256
    end
    object cdsMappingCOMPILED_NAME_OTDEL: TStringField
      FieldName = 'COMPILED_NAME_OTDEL'
      Size = 500
    end
    object cdsMappingREMARKS: TStringField
      FieldName = 'REMARKS'
      Size = 256
    end
    object cdsMappingCOLOUR: TStringField
      FieldName = 'COLOUR'
      Size = 50
    end
  end
  object dsMapping: TOraDataSource
    DataSet = cdsMapping
    Left = 160
    Top = 152
  end
  object cxStyleRepository1: TcxStyleRepository
    PixelsPerInch = 96
    object cxStyle1: TcxStyle
      AssignedValues = [svColor]
      Color = 12708863
    end
  end
end
