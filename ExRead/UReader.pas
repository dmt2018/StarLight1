...
begin
    ...
    try
      ...
      begin
        if not cdsTranslateN_ID.IsNull then cnt := cnt + 1;
        if not cdsTranslateN_ID_DESC.IsNull and cdsTranslateN_ID.IsNull then cnt_desc := cnt_desc + 1;
        if not cdsTranslateINVOICE_DATA_ID.IsNull then cnt_inv := cnt_inv + 1;
        cdsTranslate.Next;
      end;
      cdsTranslate.First;
    finally
    ...
    lbl_set.Caption  := IntToStr(cnt);
    lbl_art.Caption  := IntToStr(cnt_desc);
    lbl_name.Caption := IntToStr(cnt_inv);
    if AS_IS_POS > 0 then
      lbl_percent.Caption := '% распознания -  '+IntToStr(round((cnt/AS_IS_POS)*100));
end;
