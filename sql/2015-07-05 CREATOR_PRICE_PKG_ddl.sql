...
-- Generated 5.07.2015 11:17:30 from CREATOR@STAR_NEW
   ...
   /* */
    ...
    LOOP
        ...
        IF OLD_PRICE_COUNT > 1 then
            LOG_ERR(' Встретилось несколько цен на один товар N_ID=' || invo_row.N_ID, 0, 'LOAD_INVOICE', 'count='||OLD_PRICE_COUNT);
        end if;
        ...
        if CNT = 0 then
            ...
            VALUES(
              ...
              const_office,
              decode(vBestPrice,0,null,vBestPrice),
              invo_row.spec_price
            );
        else
...
*/
    ...
    ;
/* 2015-07-01 От заказа неверно идти для упакованных клиентов.
    insert into ppl_client_price
...
BEGIN
  ...
  open cursor_ for
    select a.ppli_id, a.ppl_id, a.coming_date, a.invoice_amount
       ...
       , nvl(pd.total_quantity,0) - nvl(pd.left_quantity,0) - nvl(d.DISTRIBUTED_NUMBER_to_zhirar,0) as DISTRIBUTED_NUMBER

       , nvl(pd.left_quantity,0) +  nvl(d.DISTRIBUTED_NUMBER_to_zhirar,0) as obshak
       , a.stock_amount, a.left_amount, a.given_amount, a.hol_price, a.ruble_price
       --, a.last_price
       , case
          when v_PPLI_ID_old = 0 then a.last_price
          else
            case
              when z.final_price is null then case when stock_amount is null then null else a.final_price end
              else z.final_price
            end
          end last_price
...
--       , case when nid_rownum=1 then a.total_profit else a.total_profit-a.stok_total_profit end total_profit
    ...
    left outer join
      (
        ...
        where p.ppli_id = v_PPLI_ID and pd.DIST_IND_ID in (select d.DIST_IND_ID from distributions_invoices d where d.inv_id in (p.inv_id, p.inv_id2, p.inv_id3, p.inv_id4) )
    ...
    left outer join
      (
        ...
        from DISTRIBUTION_VIEW c, PREPARE_PRICE_LIST_INDEX p
          where p.ppli_id = v_PPLI_ID and c.DIST_IND_ID in (select d.DIST_IND_ID from distributions_invoices d where d.inv_id in (p.inv_id, p.inv_id2, p.inv_id3, p.inv_id4) )
    ...
    left outer join
      (
        select z.n_id, max(z.final_price) as final_price
          from PREPARE_PRICE_LIST z, invoice_data d
          where z.ppli_id = v_PPLI_ID_old and z.ppli_id <> v_PPLI_ID and z.INVOICE_DATA_ID = d.INVOICE_DATA_ID and d.TO_CLIENT is null
          group by z.n_id
      ) z on z.n_id = a.n_id
/*
    left outer join prepare_distribution pd on pd.DIST_IND_ID in (
            select distinct d.DIST_IND_ID
            from distributions_invoices d, PREPARE_PRICE_LIST_INDEX p
            where p.ppli_id = 10002580 and (d.INV_ID = p.inv_id or d.INV_ID = p.inv_id2 or d.INV_ID = p.inv_id3 or d.INV_ID = p.inv_id4)
          )
          and pd.n_id = a.n_id
    left outer join
      (
        ...
        from DISTRIBUTION_VIEW c
          where c.DIST_IND_ID in (
            select distinct d.DIST_IND_ID
            from distributions_invoices d, PREPARE_PRICE_LIST_INDEX p
            where p.ppli_id = 10002580 and (d.INV_ID = p.inv_id or d.INV_ID = p.inv_id2 or d.INV_ID = p.inv_id3 or d.INV_ID = p.inv_id4)
          )
          and c.id_clients in (const_dir,const_main) and c.INVOICE_DATA_ID is not null
          group by c.d_n_id
      ) d on d.d_n_id = a.n_id
*/
    ...
    order by compiled_name_pot, len;
