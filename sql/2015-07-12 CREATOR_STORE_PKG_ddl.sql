...
-- Generated 12.07.2015 21:14:35 from CREATOR@STAR_NEW
...
*/
  ...
  if (v_is_minus = 0) then
      ...
      select sum(r.sended_to_warehouse) into cnt
        from invoice_register r, invoice_register i
        where r.inv_id = inv_num and (( r.ipp_id = i.ipp_id and r.ipp_id is not null ) or r.inv_id = i.inv_id );
      ...
      if cnt = 0 then
        ...
        -- 2014-10-02 Очистим спец.цену и предложение по истечении недели, т.к. действует это с выходных на выходные
        update price_list a set a.date_change=sysdate, a.SPEC_PRICE = null, a.BEST_PRICE = null
          where (a.SPEC_PRICE is not null or a.BEST_PRICE is not null)
            and exists (select 1 from nomenclature_mat_view n
                          where n.n_id = a.n_id
                          and n.ID_DEPARTMENTS = v_ID_DEPARTMENTS
                          and n.s_id in (
                              select distinct i.S_ID_DEFAULT
                                from invoice_register r, invoice_register i
                                where r.inv_id = inv_num and (( r.ipp_id = i.ipp_id and r.ipp_id is not null ) or r.inv_id = i.inv_id )
                          )
            )
        ...
        -- 1. Обновим цены на переоцененный товар со склада, тот что без инвойса
...
/* 2015-07-06 Не все данные стали попадать
      SELECT a.N_ID, sysdate, max(a.FINAL_PRICE) as FINAL_PRICE, const_office, sysdate, max(a.spec_price), max(a.best_price), max(a.discount)
        FROM PREPARE_PRICE_LIST a, invoice_data r
        WHERE a.PPLI_ID = v_ppli_id
            and a.n_id = r.n_id
            and r.inv_id = inv_num
            and a.invoice_data_id = r.invoice_data_id
...
--    select user into SP_OPERATOR_NAME from dual;
    ...
    if SP_ID_DOC_TYPE=4 THEN --продажа
      if (SP_ID_CLIENT is null or SP_ID_CLIENT = 0) then
         RAISE_APPLICATION_ERROR (-20305, 'Произошла непредвиденная ошибка. Получен пустой код клиента' || chr(10) || 'Пришлите скриншот разработчикам [SP_DOC_DATE='||SP_DOC_DATE||', SP_OTDEL='||SP_OTDEL||', SP_OPERATOR_NAME='||SP_OPERATOR_NAME||', DDOC_NUMBER='||DDOC_NUMBER||']');
      end if;
        ...
        begin
            ...
            into last_price_st, final_price_st
                  FROM ppl_view a, invoice_register r
                        ...
                        group by n_id
            ;
/* Взять текущую цену нельзя, т.к. она уже изменена в этой транзакции и ждет коммита
            SELECT COUNT(*) into exist FROM PRICE_LIST WHERE N_ID = data_temp_cursor.N_ID and id_office = const_office;
            LOG_ERR('last_price_st='||exist, 3, 'store_pkg.insert_in_store', '');

            IF exist > 0 then
                SELECT max(PRICE) into last_price_st FROM PRICE_LIST WHERE N_ID = data_temp_cursor.N_ID and id_office = const_office;
            else
                last_price_st := 0;
            end if;
            LOG_ERR('last_price_st='||last_price_st, 4, 'store_pkg.insert_in_store', '');
*/
        EXCEPTION
          WHEN others THEN
  ...
  select SUM( QUANTITY*PRICE )
    into DDOC_SUM
      from STORE_DOC_DATA a, nomenclature n
        where a.ID_DOC = IID_DOC and a.n_id = n.n_id and n.id_office in (1, const_office) and n.fst_id not in (const_cargo);
--      from STORE_DOC_DATA
--        where ID_DOC = IID_DOC;
...
BEGIN
  ...
  open cursor_ for
    SELECT distinct a.id_doc, a.doc_number, a.id_doc_type, a.id_departments,
       ...
       , e.office, o.D_DATE as order_date
        FROM store_doc_view2 a, (select distinct office, id_doc, id_invoice from export_to_fillials) e, orders_clients o
       ...
       , e.office
        FROM store_doc_view2 a, (select distinct office, id_doc, id_invoice from export_to_fillials) e
          where a.id_departments = id_dep_
                ...
                and (a.id_office = v_office or v_office = 0)
--                and v_cargo = 0
                and (v_cargo=0 or (v_cargo=1 and exists (select 1 from store_doc_data z, nomenclature y where z.id_doc = a.id_doc and z.n_id = y.n_id and y.fst_id =const_cargo) ))
                and a.id_doc = e.id_doc(+)
             ORDER BY DOC_DATE desc, DOC_DATE_real desc;
...
begin
    ...
    (
      select STORE_DOC_DATA_TEMP_SET_ID.NEXTVAL, a.n_id, a.QUANTITY, a.store_type, case when fix_price=1 then a.price else b.price end pr, b.price
        , get_gtd(a.n_id, const_office)
...
/
GRANT EXECUTE ON store_pkg TO eva
/
