...
-- Generated 29.03.2015 21:54:56 from CREATOR@STAR_NEW
...
begin
    ...
    open p_cursor for
        ...
        from (
            SELECT distinct a.fst_id, a.f_sub_type, a.ft_id, a.hol_sub_type, a.mnemo, a.sub_weight, a.PRICE_PREFIX, a.tnved
...
begin
   ...
   if tmp_cnt = 0 then
      INSERT INTO FLOWER_SUBTYPES VALUES(p_fst_id, p_f_sub_type, p_ft_id, p_hol_sub_type, p_mnemo, p_sub_weight, 1, sysdate, p_PRICE_PREFIX, p_tnved);
   end if;
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_inv_data('||p_inv_id||','||n_id||','||units||','''||FINAL_PRICE||''','''||total_sum||''','''||amount_in_the_pack||''','||ID_DOC_DATA||',0,'''||gtd||''',0,'''||gtd_country||''',null,0); end;'
        from (
     ...
     if v_invoice_data_id > 0 then
        if tmp_cnt = 0 then
           ...
           insert into prepare_price_list values(v_ppli_id, v_ppl_id, sysdate, p_invoice_amount, p_stock_amount, p_left_amount, p_given_amount, p_n_id, v_invoice_data_id,
                p_hol_price, p_ruble_price, p_last_price, p_price_pcc, p_price_pcc_pc, p_total_profit, p_final_price, p_col, const_office, sysdate, null, null, null );
        else
           select invoice_data_id into tmp_cnt
