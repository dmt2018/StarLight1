...
-- Generated 10.12.2011 20:54:39 from CREATOR@STARREG
...
BEGIN
  open cursor_ for
       SELECT /*+ NO_CPU_COSTING */ 0 as d_check, f_name, f_name_ru, f_sub_type, f_type, country, type_subtype
              ...
              , weight, s_name_ru, colour, is_photo, hol_marks, rus_marks, diameter, bar_code, cust_coef
              , REMARKS, photo, compiled_name_otdel
              , o.id_office, o.brief
...
BEGIN
  open cursor_ for
       SELECT /*+ NO_CPU_COSTING */ 0 as d_check, f_name, f_name_ru, f_sub_type, f_type, country, type_subtype
              ...
              , photo, great_name_f, compiled_name_otdel
    ...
    if (where_ = 0) then
       ...
       if (what_ = 0) then
          ...
          if CNT > 0 then
             ...
             INSERT INTO COUNTRIES VALUES (get_office_unique('C_ID'), NAME_, NAME2_) returning c_id into NEW_ID_;
          end if;
       -- редактирование
       else
          SELECT COUNT(*) INTO CNT from dual where exists (
          ...
          if CNT > 0 then
             ...
             UPDATE COUNTRIES SET COUNTRY = NAME_, COUNTRY_ENG = NAME2_ WHERE C_ID = ID_;
             NEW_ID_ := ID_;
          end if;
       end if;
...
BEGIN
      ...
      IF CNT > 0 THEN
        ...
        IN_ID_   := 0;
            ...
            INSERT INTO SUPPLIERS VALUES(get_office_unique('S_ID'),S_NAME_RU_,C_ID_,NEED_CUST_, const_office, sysdate) returning S_ID into S_ID_;
        COMMIT;
        IN_ID_ := S_ID_;
      END IF;
...
BEGIN
      ...
      IF CNT > 0 THEN
        ...
        in_id_ := 0;
            ...
            UPDATE SUPPLIERS SET S_NAME_RU = S_NAME_RU_, C_ID = C_ID_, NEED_CUST = NEED_CUST_, date_change=sysdate WHERE S_ID=S_ID_;
        COMMIT;
        in_id_ := S_ID_;
      END IF;
...
BEGIN
      ...
      IF CNT > 0 THEN
        ...
        in_id_ := 0;
            ...
            INSERT INTO FLOWER_SUBTYPES VALUES(get_office_unique('FST_ID'),S_NAME_RU_,FT_ID_,HOL_SUB_TYPE_,MNEMO_,wight_,const_office,sysdate) returning FST_ID into S_ID_;
        COMMIT;
        in_id_ := S_ID_;
      END IF;
...
BEGIN
      ...
      IF CNT > 0 THEN
        ...
        in_id_ := 0;
            ...
            UPDATE FLOWER_SUBTYPES SET F_SUB_TYPE = S_NAME_RU_, HOL_SUB_TYPE = HOL_SUB_TYPE_, MNEMO = mnemo_, sub_weight = wight_, date_change=sysdate WHERE FST_ID = FST_ID_;
        COMMIT;
        in_id_ := FST_ID_;
      END IF;
...
BEGIN
   ...
   IF N_ID_ = -1 then
        ...
        where FN_ID = FN_ID_ and FST_ID = FST_ID_ and S_ID = S_ID_ and C_ID = C_ID_ and H_CODE = H_CODE_ and H_NAME = H_NAME_ and
                        LEN = LEN_ and PACK = PACK_ and COL_ID = COL_ID_ and VBN = VBN_ and HOL_TYPE = HOL_TYPE_ and WEIGHT = WEIGHT_ and DIAMETER = DIAMETER_ and
                        BAR_CODE = BAR_CODE_ and CUST_COEF = CUST_COEF_ and REMARKS = REMARKS_
        ...
        where FN_ID = FN_ID_ and FST_ID = FST_ID_ and S_ID = S_ID_ and C_ID = C_ID_ and H_CODE = H_CODE_ and H_NAME = H_NAME_ and
                        LEN = LEN_ and PACK = PACK_ and COL_ID = COL_ID_ and VBN = VBN_ and HOL_TYPE = HOL_TYPE_ and WEIGHT = WEIGHT_ and DIAMETER = DIAMETER_ and
                        BAR_CODE = BAR_CODE_ and CUST_COEF = CUST_COEF_ and REMARKS = REMARKS_ and N_ID <> N_ID_
                        and id_office=const_office;
   ...
   THEN --Добавление
      ...
      bar_code13 := EAN12_TO_EAN13(bar_code12);
                ...
                sqlstr := 'INSERT INTO NOMENCLATURE (N_ID, FN_ID, FST_ID, S_ID, C_ID, H_CODE, H_NAME, LEN, PACK, COL_ID, VBN, HOL_TYPE, WEIGHT, DIAMETER, BAR_CODE, code, CUST_COEF, REMARKS, PHOTO, ID_OFFICE)
                VALUES (    N_ID.NEXTVAL, '||FN_ID_||', '||FST_ID_||', '||S_ID_||', '||C_ID_||', '||H_CODE_||', '||H_NAME_||', '||LEN_||', '||PACK_||', '||COL_ID_||', '||VBN_||', '||HOL_TYPE_||', '||WEIGHT_||', '||DIAMETER_||',
                                    '||BAR_CODE_||', '||bar_code13||', '||CUST_COEF_||', '||REMARKS_||', '||PHOTO_||','|| const_office ||')';

                INSERT INTO NOMENCLATURE (N_ID, FN_ID, FST_ID, S_ID, C_ID, H_CODE, H_NAME, LEN, PACK, COL_ID, VBN, HOL_TYPE, WEIGHT, DIAMETER,
                                        BAR_CODE, code, CUST_COEF, REMARKS, PHOTO, ID_OFFICE)
                 VALUES (   get_office_unique('N_ID'), FN_ID_, FST_ID_, S_ID_, C_ID_, trim(H_CODE_), trim(H_NAME_), LEN_, PACK_, COL_ID_, VBN_, HOL_TYPE_, WEIGHT_, DIAMETER_,
                                    trim(BAR_CODE_), bar_code13, CUST_COEF_, REMARKS_,  PHOTO_, const_office
   ...
   ELSE --Редактирование
                UPDATE NOMENCLATURE SET FN_ID = FN_ID_, FST_ID = FST_ID_, S_ID = S_ID_, C_ID = C_ID_, H_CODE = trim(H_CODE_), H_NAME = trim(H_NAME_),
                             LEN = LEN_, PACK = PACK_, COL_ID = COL_ID_, VBN = VBN_, HOL_TYPE = HOL_TYPE_, WEIGHT = WEIGHT_, DIAMETER = DIAMETER_,
                             BAR_CODE = trim(BAR_CODE_), CUST_COEF = CUST_COEF_, REMARKS = REMARKS_--, PHOTO = PHOTO_
                             , date_change = sysdate
                             WHERE N_ID = N_ID_;
...
is
begin
  select count(*) into CNT from NOM_SPECIFICATIONS a where a.n_id = N_ID_ and HS_ID = HS_ID_ and HS_VAL = HS_VAL_ and ID_OFFICE = const_office;
  if CNT = 0 then
    INSERT INTO NOM_SPECIFICATIONS (N_ID,HS_ID,HS_VAL,ORD, id_office)
                 VALUES(N_ID_,HS_ID_,HS_VAL_,(SELECT NVL(MAX(ORD),0)+1 FROM NOM_SPECIFICATIONS WHERE N_ID = N_ID_), const_office);
...
BEGIN
  ...
  DELETE FROM NOM_SPECIFICATIONS WHERE N_ID = N_ID_ AND HS_ID = HS_IDS_ and id_office=const_office;
  nomenclature2_pkg.sync_nomenclature_one(N_ID_);
  COMMIT;
...
begin
  ...
  open cursor_ for
    SELECT FN_ID, F_NAME_RU, NAME_CODE, FITO_NAME, a.id_office, o.brief
...
begin
  ...
  open cursor_ for
      SELECT a.FT_ID, a.F_TYPE, a.FT_ID as FT_ID_, a.id_office, o.brief
...
begin
  ...
  open cursor_ for
    ...
    FROM SUPPLIERS a, offices o
      where a.id_office = const_office and a.id_office = o.id_office
      ...
      IF CNT = 0 THEN
        ...
        loop
            ...
            exit when f_%notfound;
                ...
                UPDATE FLOWER_SUBTYPES set sub_weight = value_ where FST_ID = CNT;
...
BEGIN
  sql_str := 'UPDATE NOMENCLATURE SET N_ID='||N_ID_IN;
   ...
   if HOL_TYPE_IN <> '' then sql_str := sql_str || ', HOL_TYPE = '''||HOL_TYPE_IN||''''; end if;
...
/* Теперь все фотографии будут автоматически подставлятся по внутреннему артиклу
   if PHOTO_IN is not null or length(PHOTO_IN) > 0 then
      sql_str := sql_str || ', PHOTO = '''||PHOTO_IN||''''; end if;
*/
...
EXCEPTION
   WHEN OTHERS THEN
           LOG_ERR(SQLERRM, SQLCODE, 'NOMENCLATURE_PKG.UPDATE_PARAMS', '');
...
create or replace public synonym nomenclature_pkg for creator.nomenclature_pkg
/
