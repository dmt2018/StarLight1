...
-- Generated 10.12.2011 20:45:49 from CREATOR@STARREG
...
BEGIN
  open cursor_ for
       SELECT a.id_regions, a.name, a.info, a.ID_OFFICE, o.BRIEF, row_number() over(partition by 1 order by upper(a.name)) nn
...
BEGIN
  ...
  if id_ > 0 then
     ...
     if (cnt_ > 0) then
        ...
        update books_regions set NAME = name_, info = info_, DATE_CHANGE=sysdate where ID_REGIONS = id_;
     end if;
     ...
     if (cnt_ > 0) then
        ...
        insert into BOOKS_REGIONS values(BOOKS_ID_REGIONS.NEXTVAL, name_, info_, CONST_OFFICE, sysdate) returning id_regions into id_;
     end if;
...
BEGIN
  if id_ > 0 then
     ...
     if (cnt_ > 0) then
        ...
        update BOOKS_CLIENT_TYPES set NAME = name_, info = info_, T_TYPE = TTYPE_, DISCOUNT = price_part_, DATE_CHANGE=sysdate where ID_CLIENT_TYPES = id_;
     end if;
    ...
    if (cnt_ > 0) then
       ...
       insert into BOOKS_CLIENT_TYPES values(BOOKS_ID_CLIENT_TYPES.NEXTVAL, name_, info_, TTYPE_, price_part_, CONST_OFFICE, sysdate) returning ID_CLIENT_TYPES into id_;
    end if;
  end if;
  commit;
...
create or replace public synonym BOOKS for creator.BOOKS
/
