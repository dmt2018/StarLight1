...
-- Generated 16.12.2011 23:00:03 from CREATOR@STARREG
...
begin
    ...
    open p_cursor for
        ...
        from (
            SELECT distinct a.fn_id, replace(a.f_name_ru,chr(39),chr(39)||chr(39)) as f_name_ru, a.id_departments, a.name_code, replace(a.fito_name,chr(39),chr(39)||chr(39)) as fito_name
...
begin
   ...
   if tmp_cnt = 0 then
      --SELECT get_office_unique('FN_ID') INTO tmp_cnt FROM DUAL;
...
begin
    ...
    open p_cursor for
        ...
        from (
            SELECT distinct a.fn_id, replace(a.f_name,chr(39),chr(39)||chr(39)) as f_name, a.name_code, a.id_departments, a.fnt_id, a.default_translation, a.remarks
...
begin
   ...
   if tmp_cnt = 0 then
      --SELECT get_office_unique('FNT_ID') INTO tmp_cnt FROM DUAL;
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_fstype('||fst_id||','''||f_sub_type||''','||ft_id||','''||hol_sub_type||''','''||mnemo||''','''||sub_weight||'''); end;'
        from (
            SELECT distinct a.fst_id, a.f_sub_type, a.ft_id, a.hol_sub_type, a.mnemo, a.sub_weight
...
begin
   ...
   if tmp_cnt = 0 then
      INSERT INTO FLOWER_SUBTYPES VALUES(p_fst_id, p_f_sub_type, p_ft_id, p_hol_sub_type, p_mnemo, p_sub_weight, 1, sysdate);
   end if;
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_countries('||c_id||','''||country||''','''||country_eng||'''); end;'
        from (
            SELECT distinct a.c_id, a.country, a.country_eng
...
begin
   ...
   if tmp_cnt = 0 then
      INSERT INTO COUNTRIES VALUES (p_c_id,p_country,p_country_eng);
   end if;
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_suppliers('||s_id||','''||s_name_ru||''','||c_id||','||need_cust||'); end;'
        from (
            SELECT distinct a.s_id, a.s_name_ru, a.c_id, a.need_cust
...
begin
   ...
   if tmp_cnt = 0 then
      INSERT INTO suppliers VALUES (p_s_id,p_s_name_ru,p_c_id,p_need_cust,1,sysdate);
   end if;
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_nomenclature('||n_id||','||fn_id||','||fst_id||','||s_id||','||c_id||','''||h_code||''','''||h_name||''','''||len||''','''||pack||''','||col_id||','||vbn||','''||hol_type||''','''||weight||''','''||bar_code||''','''||code||''','''||diameter||''','''||cust_coef||''','''||remarks||''','''||photo||''','''||h_namecode||'''); end;'
        from (
            SELECT distinct a.n_id, a.fn_id, a.fst_id, a.s_id, a.c_id, a.h_code, replace(replace(a.h_name,chr(39),chr(39)||chr(39)),chr(10),'') as h_name,
               ...
               a.code, a.diameter, a.cust_coef, a.remarks, a.photo, a.h_namecode
...
begin
   ...
   if tmp_cnt = 0 then
      INSERT INTO NOMENCLATURE (N_ID, FN_ID, FST_ID, S_ID, C_ID, H_CODE, H_NAME, LEN, PACK, COL_ID, VBN, HOL_TYPE, WEIGHT, DIAMETER, BAR_CODE, code, CUST_COEF, REMARKS, PHOTO, H_NAMECODE)
      VALUES (p_n_id, p_fn_id, p_fst_id, p_s_id, p_c_id, p_h_code, p_h_name, p_len, p_pack, p_col_id, p_vbn, p_hol_type, p_weight, p_diameter,
                                   p_bar_code, p_code, p_cust_coef, p_remarks,  p_photo, p_namecode);
   end if;
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_inv_register(0,to_date('''||inv_date||''',''dd.mm.yy''),'''||comments||''',null,'||sended_to_warehouse||','''||ipp_id||''','||id_departments||','''||id_orders||''',1,'''||c_id_default||''',null,sysdate,'''||inv_id||''','||inv_minus||','||minus_inv_id||',null); end;'
        from (
...
begin
  select id_departments, DOC_DATE, doc_number into v_id_dep, v_ddate, v_doc_number from store_doc where id_doc = p_inv_id;
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_inv_register(0,to_date('''||v_ddate||''',''dd.mm.yy''),''Инвойс из накладной №'||v_doc_number||''',null,0,null,'||v_id_dep||',null,1,230,null,sysdate,'''||p_inv_id||''',0,null,null); end;'
        from dual
        ;
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_inv_data('||inv_id||','||n_id||','||units||','''||FINAL_PRICE||''','''||total_sum||''','''||amount_in_the_pack||''',0,'||invoice_data_id||',0,'''||gtd||''',0); end;'
        from (
            SELECT distinct a.inv_id, a.n_id, a.units, nvl(c.FINAL_PRICE,0) as FINAL_PRICE, case when c.FINAL_PRICE is null then 0 else round((c.FINAL_PRICE * a.units),2) end total_sum,
                a.amount_in_the_pack, a.invoice_data_id, a.invoice_data_as_is_id,  a.sended_to_warehouse, a.gtd, a.storned_data
              FROM invoice_data a
...
begin
    ...
    open p_cursor for
        select 'begin sync_pkg.ins_inv_data('||p_inv_id||','||n_id||','||units||','''||FINAL_PRICE||''','''||total_sum||''','''||amount_in_the_pack||''',0,'||ID_DOC_DATA||',0,'''||gtd||''',0); end;'
        from (
   ...
   if v_inv_id > 0 then
     ...
     if tmp_cnt = 0 then
       ...
       INSERT INTO prepare_price_list_index
            ...
            v_inv_id, null, null, null, p_pack_id, const_office, sysdate);
     end if;
   end if;
     ...
     if v_invoice_data_id > 0 then
        if tmp_cnt = 0 then
           ...
           insert into prepare_price_list values(v_ppli_id, v_ppl_id, sysdate, p_invoice_amount, p_stock_amount, p_left_amount, p_given_amount, p_n_id, v_invoice_data_id,
                p_hol_price, p_ruble_price, p_last_price, p_price_pcc, p_price_pcc_pc, p_total_profit, p_final_price, p_col, const_office, sysdate );
        else
           select invoice_data_id into tmp_cnt
...
create or replace public synonym sync_pkg for creator.sync_pkg
/
