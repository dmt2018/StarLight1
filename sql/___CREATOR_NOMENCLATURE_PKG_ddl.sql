...
-- Generated 29.03.2015 21:54:38 from CREATOR@STAR_NEW
...
BEGIN
      ...
      IF CNT > 0 THEN
        ...
        in_id_ := 0;
            ...
            INSERT INTO FLOWER_SUBTYPES VALUES(get_office_unique('FST_ID'),S_NAME_RU_,FT_ID_,HOL_SUB_TYPE_,MNEMO_,wight_,const_office,sysdate, prefix_, vTnVed) returning FST_ID into S_ID_;
        COMMIT;
        in_id_ := S_ID_;
      END IF;
...
BEGIN
      ...
      IF CNT > 0 THEN
        ...
        in_id_ := 0;
            ...
            UPDATE FLOWER_SUBTYPES SET F_SUB_TYPE = S_NAME_RU_, HOL_SUB_TYPE = HOL_SUB_TYPE_, MNEMO = mnemo_, sub_weight = wight_, date_change=sysdate, PRICE_PREFIX=prefix_, TnVed=vTnVed
            WHERE FST_ID = FST_ID_;
        COMMIT;
        in_id_ := FST_ID_;
      END IF;
...
BEGIN
   ...
   IF N_ID_ = -1 then
        ...
        where FN_ID = FN_ID_ and FST_ID = FST_ID_ and S_ID = S_ID_ and C_ID = C_ID_ and H_CODE = H_CODE_ and H_NAME = REPLACE(H_NAME_,'''','`') and
                        ...
                        and DIAMETER = DIAMETER_ and BAR_CODE = BAR_CODE_ and CUST_COEF = CUST_COEF_ and REMARKS = REMARKS_
...
--LOG_ERR(SQLERRM, SQLCODE, 'NOMENCLATURE_PKG.EDIT_NOM', sqlstr);
                ...
                INSERT INTO NOMENCLATURE (N_ID, FN_ID, FST_ID, S_ID, C_ID, H_CODE, H_NAME, LEN, PACK, COL_ID, VBN, HOL_TYPE, WEIGHT, DIAMETER,
                                        BAR_CODE, code, CUST_COEF, REMARKS, PHOTO, ID_OFFICE, noprint, notuse, TnVed, weightdry, nom_new, nom_start, nom_end, H_NAMECODE, HOL_COLOR)
                 VALUES (   get_office_unique('N_ID'), FN_ID_, FST_ID_, S_ID_, C_ID_, trim(H_CODE_), trim(REPLACE(H_NAME_,'''','`')), LEN_, PACK_, COL_ID_, VBN_, HOL_TYPE_, WEIGHT_, DIAMETER_,
                                    trim(BAR_CODE_), bar_code13, CUST_COEF_, REMARKS_,  PHOTO_, const_office, noprint_, notuse_, vTnVed, weightdry_, nom_new_, nom_start_, nom_end_, REPLACE(name_code_,'''','`'), hol_color_
         ...
         select
          ...
          , nom_new_, nom_start_, nom_end_, b.hol_color
...
BEGIN
  ...
  INSERT INTO NOMENCLATURE
      (SELECT get_office_unique('N_ID'), FN_ID, FST_ID, S_ID, C_ID, H_CODE, H_NAME, LEN, PACK, COL_ID, VBN, HOL_TYPE,
                ...
                , noprint, notuse, TnVed, weightdry, nom_new, nom_start, nom_end, hol_color
...
BEGIN
  sql_str := 'UPDATE NOMENCLATURE SET N_ID='||N_ID_IN;
   ...
   sql_str := sql_str || ', noprint = '||noprint_;
   sql_str := sql_str || ', notuse = '||notuse_;
   sql_str := sql_str || ', NOM_NEW = '||nom_new_;
   sql_str := sql_str || ', NOM_START = '||nom_start_;
   sql_str := sql_str || ', NOM_END = '||nom_end_;
...
BEGIN
  if param_IN = '1' then
    ...
    insert into nom_specifications values(N_ID_IN, const_8march, '1', 1, const_office, sysdate);
  end if;
  nomenclature2_pkg.sync_nomenclature_one(N_ID_IN);
  commit;
...
/
GRANT EXECUTE ON nomenclature_pkg TO eva
/
