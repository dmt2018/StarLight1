...
//    days_minus := RegIni.ReadInteger('days_minus','value', 5);
  ...
  RegIni := TIniFile.Create(path+'..\orders\orders.ini');
  try
...
begin
  ColIni := TIniFile.Create(path+'/'+Main_session.Username+'_.ini');
  edPath.Text := ColIni.ReadString('folder','value',path);
end;
...
begin
        print_price(path + '/Reps/PriceRep.fr3', 'QUANTITY > 0')
