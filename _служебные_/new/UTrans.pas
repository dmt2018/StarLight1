...
if selq.FieldByName('p1').AsInteger>0 then begin
  ...
  Q_TRANS.Refresh
end else showmessage('нельз€ удалить запись');
except
  on E: Exception do ShowMessage('ќшибка! ' + E.Message);
end;
end;
...
try
  ...
  if Q_TRANS.FieldByName('STAT').asstring='0'  then  frmEditTrans.cxcombobox1.Text := 'прин€т';
...
begin
  if MessageDlg('«акрыть программу?',mtConfirmation,[mbYes, mbNo],0) <> mrYes then
   Action := caNone
  else
  begin
...
begin
  try
      ...
      if (str_tmp = 0) then str_text := str_text + 'прин€т к рассмотрению' else
