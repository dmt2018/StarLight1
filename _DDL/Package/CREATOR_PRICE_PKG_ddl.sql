...
--
--  Установка подключения цен прошлого инвойса для графы предыдущая цена
--
...
--
--  Синхронизация цен с текущим прайс-листом
--
...
/* 2015-11-28 Е.В., обновлять надо только добавленные позиции в инвойсе
  ...
  ;
*/
/* 2015-12-15 Е.В., Обновлять надо добавленные и удаленные позиции */
  delete from PREPARE_PRICE_LIST a
  where a.ppli_id = IN_PPLI_ID and a.invoice_data_id is not null
    ...
    from store_main b, PRICE_LIST d, NOMENCLATURE e, (
       select sum(a.UNITS) as UNITS, a.N_ID, a.PRICE_PER_UNIT, max(a.INVOICE_DATA_ID) as INVOICE_DATA_ID
        ...
        where a.STORNED_DATA = 0
          and a.to_client is null  -- добавим условие на упакованного, т.к. цена в прайсе уже другая
          and a.INV_ID in
          (
            select r.inv_id from prepare_price_list_index i, invoice_register r
...
IS
BEGIN
  -- удалим записи, где уже нет остатков на складе по подгруженному стоку
  delete from PREPARE_PRICE_LIST a
    where a.ppli_id = IN_PPLI_ID
...
EXCEPTION
   ...
   THEN
      ...
      RAISE_APPLICATION_ERROR (-20426, 'Запрос не выполнился. ' || SQLERRM);
...
EXCEPTION
   ...
   THEN
      ...
      RAISE_APPLICATION_ERROR (-20427, 'Запрос не выполнился. ' || SQLERRM);
