...
//
//  Вначале выбираем что делать с прайсами (подгрузить, выбрать и т.п.)
//
...
Begin
  ...
  while mod_res do
    Begin
      ...
      DM.PPL.Open;
      Caption := 'Прайс-лист. ИД-'+IntToStr(PPLI_ID);
      ShowModal;
      DM.PPL.Filter := '';
...
begin
  ...
  InvoChoose.Caption := 'Выбор инвойса для работы';
  ...
  try
    ...
    begin
      if MessageDlg('Подгрузить всю пачку №'+DM.InvoRegIPP_ID.AsString+#10+DM.InvoRegIPP_COMMENT.AsString+'?', mtConfirmation, [mbNo,mbOk], 0, mbOk) = mrOk then
      begin
...
{
BOF: Проверка на наличие нераспознанных строк
}
    cnt     := 0;
    err_msg := '';
    with DM.SelQ do
    Begin
      ...
      if (cnt > 0) then
      begin
        if MessageDlg('Выбраны инвойсы у которых есть нераспознанные записи!'+#10+'Нажмите ДА если хотите скопироать остатки в новый инвойс, НЕТ - оставить все как есть', mtConfirmation, [mbNo,mbOk], 0, mbOk) = mrOk then
        begin
          Close;
          SQL.Clear;
    ...
    End;
{
EOF: Проверка на наличие нерспознанных строк
}
    ...
    // Вставим спец цены на правильных клиентов 
        with DM.SelQ do
        begin
          Close;
          SQL.Clear;
    ...
    Label6.Caption := '№ инвойсов: '+ DM.PPL_Index.fieldByName('ALL_INV').AsString;
    Label8.Caption := '№ пачки: '+ VarToStr(DM.PPL_Index.fieldByName('PACK_ID').Value);
...
begin
  ...
  Begin
    ...
    // Соберем все подтипы для фильтра стока, если заполнено
    ...
    // Соберем всех поставщиков для фильтра стока, если заполнено
...
begin
  ...
  begin
      MessageBox(Handle, 'Обнаружена нулевая цена продажи!'+#10+'Сохранение невозможно.', 'Внимание!', MB_ICONWARNING);
      exit;
  end;
...
begin
  if (MessageDlg('Будут пересчитаны все установленные цены с учетом коэффициентов!'+#10+'Продолжить операцию?', mtConfirmation, [mbNo,mbOk], 0, mbNo) = mrNo) then
    exit;
...
//
//  На показе формы восстанавливаем грид
//
...
//
//  На закрытие формы сохраняем грид
//
...
begin
  ...
  if is_sync then
  begin
    if (MessageDlg('Сохранить изменения цен в связи с синхронизацией?', mtConfirmation, [mbNo,mbOk], 0, mbNo) = mrNo) then
      DM.STAR_DB.Rollback
    else
      DM.STAR_DB.Commit;
  end
...
//
// Красим одинаковые позиции с разными ценами
//
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // Красим позиции у которых цена продажи = цене закупки
...
begin
  try
    ...
    frx_price.Variables['PrintMark'] := '''Отдел: ' + CUR_DEPT_NAME + '''';
...
begin
  ...
  Begin
    if MessageDlg('Все данные о переоценки будут потеряны! Выгрузить сток?', mtConfirmation, [mbNo,mbOk], 0, mbOk) = mrOk then
    begin
      with DM.ForceQ do
      Begin
        Close;
...
begin
  if (DM.PPL.Active = true) and (DM.PPL.RecordCount > 0) then
  Begin
    if MessageDlg('Будут синхронизированы остатки по складу! Продолжить операцию?', mtConfirmation, [mbNo,mbOk], 0, mbOk) = mrOk then
    begin
      with DM.ForceQ do
      Begin
        Close;
        SQL.Clear;
        SQL.Add('begin price_pkg.sync_ppli(:in_ppli_id); end;');
        Prepare;
        ParamByName('IN_PPLI_ID').Value := DM.PPL_Index.fieldByName('PPLI_ID').Value;
        Execute;
      End;
...
//
// Синхронизируем инвойс
//
procedure TPriceF.btnSyncInvClick(Sender: TObject);
begin
  if (DM.PPL.Active = true) and (DM.PPL.RecordCount > 0) then
  Begin
    if MessageDlg('Будут синхронизированы все данные по подгруженным инвойсам! Продолжить операцию?', mtConfirmation, [mbNo,mbOk], 0, mbOk) = mrOk then
    begin
      with DM.ForceQ do
      Begin
        Close;
...
// Загрузка списка клиентов в комбо
...
// Просмотр списка заказавших данную позицию
...
//
//  Установка единой цены на торар по критерию ---------------------------------
//

// Установка общей цены для выбранных записей
...
begin
  ...
  begin
    ...
    te_price.EditValue := FINAL_PRICE.EditValue;
    te_price.SetFocus;
  end;
end;
...
begin
  ...
  begin
    ...
    if MessageDlg('Будет установлена единая цена '+FloatToSTr(price)+' на выбранные позиции! Продолжить операцию?', mtConfirmation, [mbNo,mbOk], 0, mbOk) = mrOk then
    begin
      while not DM.PPL.Eof do
      begin
        DM.PPL.Edit;
...
//
//  Установка признака предложения ---------------------------------
//
...
begin
  ...
  begin
    ...
    if filter_param = 'bsProfit' then grid_pplView1LOSS_PROFIT.DataBinding.AddToFilter(nil, foEqual, 1,'ДА', true);
...
// Подгрузим в цены предыдущие цены инвойса
...
// Синхронизация цен с текущим прайс-листом фирмы
...
// Синхронизация цен с предыдущим прайсом (выбранным из списка)
...
begin
  if (MessageDlg('Будут пересчитаны все установленные цены с учетом текущего или выбранного праса!'+#10+'Продолжить операцию?', mtConfirmation, [mbNo,mbOk], 0, mbNo) = mrNo) then
    exit;
...
begin
  if (MessageDlg('Синхронизация будет отменена!'+#10+'Продолжить операцию?', mtConfirmation, [mbNo,mbOk], 0, mbNo) = mrNo) then
    exit;
